...
>
> **状态：** 已立项（2026-08-31 产品确认）。GACP-03 / P0-19g / P0-19a / GACP-06 / P0-19b / P0-19c / P0-19d / P0-19e 代码已落地（开发版 GUI 未过）。P0-19f 任务 1–5 与 overlay/停止芯片已落地；**虚拟鼠标硬验收因观察阻塞保持开放**（ACP 指针键 not-observed，`click_at` 为 viewport-css）；开发版 GUI 未过；不得标计划完成。下一步 [P0-21](p0-21-host-managed-browser.md)（计划已写）。`screen` / `clipboard` 留给后置软件表面 / P3-07，不留给 19f。
>
...
| 浏览器、Computer Use | 插件 browser L3、截图 Artifact、停止 HUD/overlay 芯片已落地；虚拟鼠标未画出（ACP 指针 not-observed / viewport-css）；GUI 未走查 | 插件**浏览器**表面 [P0-19f](p0-19f-browser-computer-use-surface.md)（虚拟鼠标硬验收仍开放；不做 screen/软件）；Codex 式共享页 [P0-21](p0-21-host-managed-browser.md)（2026-09-04 确认，取代 P3-05）；点桌面软件仍后置 P3-07 |
...
- 本程序 19a–e 不做共享 BrowserView。P0-21 的 WebContentsView 禁止任意 CDP 转发、禁止读用户 Chrome Profile，**内置页仍不画光标**。系统级 macOS 虚拟光标（Accessibility / CGEvent）仍禁止，见 P3-07。P0-19f 只为**浏览器插件**做点击穿透的置顶 HUD；当前只落地停止芯片，虚拟鼠标因坐标不可映射未画出，不移动系统指针，也不打开 `screen` / `clipboard`。
- 不为 Codex 复制本程序的表面。
