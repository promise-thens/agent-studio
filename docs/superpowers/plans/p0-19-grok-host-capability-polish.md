...
| 浏览器、Computer Use | 插件 browser L3、截图 Artifact、停止 HUD/overlay 芯片已落地；插件虚拟鼠标未画出（ACP 指针 not-observed / viewport-css）；GUI 未走查 | 插件**浏览器**表面 [P0-19f](p0-19f-browser-computer-use-surface.md)（插件虚拟鼠标硬验收仍开放；不做 screen/软件）；Codex 式共享页 [P0-21](p0-21-host-managed-browser.md)；宿主页软件光标 [P0-21a](p0-21a-host-browser-pointer-and-snapshot.md)（代码已落地，GUI 未走查）；点桌面软件仍后置 P3-07（消费 overlay，不自建光标窗） |
