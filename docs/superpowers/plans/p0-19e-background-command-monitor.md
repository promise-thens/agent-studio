...
>
> **状态：** 任务 1–3 代码已落地。聚焦 Vitest 已过；开发版 GUI 未走查。
>
...
- [x] **第 1 步: 停止仍走 Task 级 cancel**

说明：测试：busy 时点停止，不调用 terminal IPC（若 P0-15 尚未存在，则断言没有新 channel）。

- [x] **第 2 步: 走查**

说明：后台测试命令（sleep）在 Timeline 标后台；停 Task 后状态不是永远 running；输出可在命令证据里看到有限文本。本会话用单元测试覆盖走查意图，**不得宣称开发版 GUI 已过**。

## 验收标准

- [x] 无稳定字段时行为与今天一致，不乱分组。
- [x] 有字段时折叠态可见后台 + 状态。
- [x] Agent 输出不进用户终端标签。
- [ ] 自动验证 + 开发版走查。（自动验证已过；开发版 GUI 未走查）
