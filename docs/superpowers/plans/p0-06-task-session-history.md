...
**核心数据流：** 用户选择目录后主进程解析 canonical root 并注册 Project；创建 Task 时固定 Project、Runtime 和 Local 执行环境，每个 Turn 单独保存实际 `modelId` / `displayName` 快照；Turn 的脱敏事件、状态和 Artifact 引用写入版本化本地存储；重新打开先恢复桌面记录，再由 Adapter 独立判断能否恢复 Runtime session。
...
- 创建 `src/main/project/project-registry.ts`、`src/main/storage/atomic-json-file.ts`、`src/main/agent/task-store.ts`、`src/main/agent/task-ipc.ts` 及就近测试。
- 创建 `src/shared/task-history.ts`，并修改 `src/shared/agent-ipc.ts`、`src/shared/app-ipc.ts`，新增 Project / Task / Turn / Runtime 引用和历史 DTO。
...
- `TaskRecord`：`taskId`、`projectId`、标题、runtimeId、权限策略引用、Local 执行环境引用、私有 RuntimeSessionRef、状态、创建/更新时间。
- `TurnRecord`：`turnId`、`taskId`、本轮实际 `modelId` / `displayName` 快照、经本地敏感信息处理且有界的用户输入展示文本、开始/结束时间、终态、规范化展示事件、Usage、验证和 Artifact 引用；不把该文本作为重启后自动执行载荷。
...
- [x] **第 1 步: 定义 canonical Project 身份**
...
- `src/shared/task-history.ts`、`AgentService`、`task-store.ts` 和 IPC DTO。

**前置依赖：**

- 依赖任务 1 的 ProjectRecord。

- [x] **第 1 步: 建立 TaskRecord 与 TurnRecord**
      说明：Task 创建时固定 `projectId`、runtimeId、权限策略和 Local 执行环境；每次启动 Turn 新建 TurnRecord 并保存当轮实际模型快照，终态后不可再改写其身份字段。
      预期：同 Task 多轮按时间排序；Task 标题可单独更新，但历史 Turn 的模型和环境事实不会随当前设置变化。

- [x] **第 2 步: 区分 Task 与 RuntimeSessionRef**
...
- `task-store.ts`、`task-ipc.ts`、原子 JSON、脱敏投影和存储测试。
...
- [x] **第 1 步: 定义持久化白名单与上限**
      说明：每个对话（Task）256 MiB，超限淘汰本对话最旧终态 Turn，不删其它对话。最多 100 个 Project、500 个 Task、每 Task 100 个 Turn。长任务不按本轮条数或体积截断；单事件仍限 256 KiB（超限只跳过该条，后续继续落盘）。用户输入最多 64 KiB，事件块最多 50 条或 512 KiB。sequence 为 Turn 内序号，唯一键为 `(taskId, turnId, sequence)`。
...
- [x] **第 3 步: 实现清理语义**
      说明：支持删除单 Task 记录、按项目移除历史索引；对话超 256 MiB 时淘汰本对话最旧终态 Turn。运行中 Turn、Worktree 和项目文件不随历史清理删除。其它对话不因容量互相挤占。
...
- [x] Project、Task、Turn、RuntimeSessionRef 和 ExecutionEnvironmentRef 的所有权及关系清晰，同一 Task 可以持久化多个 Turn。
- [x] 实时展示所需的事件 kind、稳定身份、sequence、状态和截断信息以版本化有界结构保存，历史回放不依赖含义模糊的“摘要”。
- [x] 应用重启后可列出并审阅历史；未完成 Turn 被标记为 `interrupted`，不会伪装成仍在运行。
- [x] 重新打开历史不依赖 Runtime；继续执行重新验证当前能力并依次尝试 `resume` 与安全条件下的 `load`，失败后仍保留只读记录。
- [x] API Key、原始环境、Runtime raw payload、屏幕和剪贴板不会落盘；单条损坏被隔离，未知新 schema 原位保留且不被旧客户端重写。
- [x] 删除历史只删除 Agent Studio 本地记录，不删除项目文件、Git Worktree 或 Runtime 原生记录，并且影响范围可观察。
- [x] 目标 ESLint、相关 Vitest、`pnpm typecheck`、`pnpm build` 与 `git diff --check` 通过，Electron 开发版完成注册项目、新建多轮 Task、重启打开和恢复失败走查。

## 当前验证快照（2026-08-12）

- 自动验证已通过：Node.js `v22.22.0`、pnpm `10.33.0`、完整 ESLint、30 个文件 / 201 个 Vitest、typecheck、build、`build:unpack`、`git diff --check`、AGENTS/CLAUDE 一致性和 GitNexus `detect_changes`。
- 专项覆盖已包含原子 JSON 顺序与失败、Project/Task quarantine、未知 schema 保留、事件分块与 256 KiB 截断、deleting 重试、Task IPC、Renderer 分页、disconnect 持久化收束和重启 `resume → load`。
- Electron 真机验收已完成：使用隔离 Project 和本机已保存 Provider 凭据，验证重复路径归一化、同 Task 多 Turn、Task A → B → A、重启只读、`resume` 成功续接、失效 session 恢复失败降级、Project 不可用、`waiting-permission` 异常退出收束为 `interrupted`、用户取消收束为 `cancelled`、损坏 Task 原子移入 quarantine，以及 Task / Project 本地历史物理删除。
- 删除验收确认项目目录、`sentinel.txt`、Git Worktree 和 Runtime 原生历史均未被删除；测试期间未读取、打印或复制 Provider Key，未批准的 `interrupt-probe.txt` 未创建。
