...
- [x] **第 1 步: 识别子 Agent 根**
      ...
      预期：切 Task、重启回放树形一致。
      落地：reducer 产出 `kind: 'agent-group'`；乱序/晚到终态由合成 `parentId` 单测覆盖。

- [x] **第 2 步: 无父子字段的降级**
      说明：保持扁平 tool 卡，不根据中文标题聚类。
      预期：普通 Turn 不出现空的「子 Agent」壳。
      落地：观察白名单为空时生产路径保持扁平；无字段不猜树。
...
- [x] 有父子字段时，时间线是嵌套卡片而不是平铺工具。
      说明：合成 `parentId` 测试已覆盖嵌套路径；真机观察白名单仍空，故生产路径保持扁平，不得宣称真机已嵌套。
- [x] 没有字段时不乱分组，功能不回退。
- [x] 子 Agent 不出现在侧栏 Task 列表。
- [x] 权限仍走同一 Broker。
- [x] 实时与历史同一棵树。
- [x] 相关测试、`pnpm typecheck`、`pnpm build`、`git diff --check` 通过。
      说明：2026-09-01 本波次 `pnpm test` 136 files / 1271 tests、`pnpm build`、`git diff --check` 已过；开发版 GUI 未走查。
