...
>
> **状态：** 代码已落地（前置：P0-11；GACP-01 已关闭，本轮只冻结到一次 `execute` + 唯一 `allow_once` / `reject_once`）。权限 E2E / 真 Grok 走查未跑。
>
> **插入点：** P0-11 之后
>
...
- [x] 项目内连续读取不出现审批卡。
- [x] 本 Task 第一次写文件最多一张卡，默认「本任务允许」；之后同类写零点击。
- [x] 项目内普通删除与写文件同级：本任务一次授权后自动过。
- [x] 越界删除、删 `.git` / 整个 root、危险 Git、未知出网、Computer Use 不会被普通写/删授权捎带通过。
- [x] 从不向 ACP 发送 `allow_always`。
- [x] 自动过的操作都有 Audit。
- [x] `rawInput` 仍不能让风险降级或自动过。
- [x] 相关 Vitest（policy / broker / mapper / 对话权限卡）全绿。
- [ ] 既有权限 E2E 全绿；真实 Grok 至少走通「读一堆 + 写几次只点一次」。
