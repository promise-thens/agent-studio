...
>
> **状态：** 任务 1–2 代码已落地。聚焦 Vitest 已过；开发版 GUI 未走查。
>
...
说明：无 hooks 时空态；放一个 http hook 只见 origin；symlink 到 grok-home 外显示 invalid。代码已落地，开发版 GUI 未走查。

## 验收标准

- [x] Renderer 与日志都看不到钩子命令原文和 query Secret。
- [x] 逃逸路径不能被当成已启用。
- [x] 桌面进程不会因为打开设置页而执行钩子。
- [ ] 自动验证已过；开发版走查未跑。
