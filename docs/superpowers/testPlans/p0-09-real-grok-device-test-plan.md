...
- 夜间补测结论：RG-09-01 同一 Task 两轮 + 历史回放改为**通过**。RG-09-02 允许一次改为**通过**（真实 Grok 申请的是 `execute-command` L3，不是 `write-file` L1；允许后写入 `scratch/p009-allow.txt`）。RG-09-05、RG-09-07、RG-09-08 改为**通过**。RG-09-03、RG-09-04、RG-09-06 保持或改为**受限通过**。Windows 窗口销毁/重建与安全历史截断仍记限制。P0-09 真机测试门按受限关闭。GACP-01 已于 2026-08-19 随后受限关闭；可以开始 GACP-02，不可开始 P0-10 主体。
