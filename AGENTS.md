...
- 最近验证：2026-09-16 P0-21a。聚焦 agent-pointer-overlay / host-browser / plugin overlay 测试已过；eslint（含 preload/overlay）/ typecheck / build / `git diff --check` 已过；开发版 GUI 未走查；插件仍不画光标；无 CGEvent；不得宣称内置页光标可用、插件虚拟鼠标完成或任意 App Computer Use 完成
