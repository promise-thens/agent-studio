...
- 状态：桌面定位为 Grok Build ACP Client，不自己当 Agent。设置现为供应商 / 外观 / 记忆 / Grok 配置 / Hooks；插件页三栏为插件 / MCP / 技能，插件 tab 内分已安装 / 市场，不得打断运行中 Task。Inspector 现为 Timeline/Plan/Changes/Terminal/Artifacts，支持悬浮拖拽、右侧吸附后三列自适应与双栏审阅；Changes 与 Artifacts 打开时加宽为审阅工作区。宿主内置浏览器计划为工作区右栏，不新增 Inspector 标签。P2 Codex 暂缓
