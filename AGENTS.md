...
- 下一步：P0-21 宿主内置浏览器（任务 2–4 代码已落地：右栏壳、动作引擎、MCP 注入；开发版 GUI 未走查；任务 5 HUD/映射未做；19f 已打开 `browser` L3，后到者不得改回 unsupported；内置页仍不画光标）。P0-19f 虚拟鼠标硬验收仍开放。P0-19e / P0-19d / P0-19c 开发版 GUI 走查仍未跑。P0-19g / P0-19a / GACP-06 / P0-19b 开发版 GUI 走查仍未跑。P0-10C 至 P0-13 开发版走查暂时可以通过。不要加回「继续任务」按钮。文件编辑器另开计划，尚未立项
- 最近验证：2026-09-07 P0-19f。聚焦 permission / mapper / origin / overlay 测试已过；typecheck / build / `git diff --check` 已过；`screen` / `clipboard` 仍 deny；无 CGEvent；开发版 GUI 未走查；虚拟鼠标未画出，不得宣称 GUI 已过或计划完成
