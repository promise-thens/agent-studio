...
- 兼容边界：首版仍只有一个执行槽；GROK_HOME 仍是 App 目录；记忆整棵树 junction 到 `~/.grok/memory`；用户 toml 只允许合并/删除 `[mcp_servers.*]`；`clientCapabilities: {}`；不把桌面做成 MCP Host 或 Marketplace Host。自动文件恢复仅 latest-turn 且无漂移；检查点只存哈希，禁止 git reset/checkout。完全接管（always-approve）按 P0-19g：当前 Task 显式开关，默认关，不写全局 yolo；确认后零确认卡，Grok 再问也由桌面 allow-once，禁止为此把 applied 打回 false（二次 toggle 会关掉接管）。Plan 开关只消费广告的 `name === 'plan'`，未广告不得伪造 `/plan`。子 Agent：`parentId` 或结构化 `[subagent:` 才成卡，禁止用中文标题猜树；不得宣称已支持任意协议嵌套。Grok 沙箱是内核 `--sandbox`，不是 Electron `webPreferences.sandbox`，也不是 Broker 替代；只读/严格不承诺挡住记忆 junction。后台命令停止走现有 `agent:cancel-turn`，不向用户 PTY 写 Ctrl+C，也不单停一条后台命令；输出进 P0-11 证据，不进 Inspector terminal
- 下一步：P0-19f 插件操作浏览器（browser L3、截图、停止条、方案 A 虚拟鼠标硬验收；不打开 screen/clipboard，不注入系统鼠标）。P0-19e / P0-19d / P0-19c 开发版 GUI 走查仍未跑。P0-21 宿主内置浏览器计划已写。P0-19g / P0-19a / GACP-06 / P0-19b 开发版 GUI 走查仍未跑。P0-10C 至 P0-13 开发版走查暂时可以通过。不要加回「继续任务」按钮。文件编辑器另开计划，尚未立项
- 最近验证：2026-09-07 P0-19e。聚焦 cancel/evidence/ToolRow chrome 测试已过；停止仍走 `agent:cancel-turn`，不碰用户 PTY；开发版 GUI 未走查，不得宣称 GUI 已过
