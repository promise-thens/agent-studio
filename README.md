...
- 标题栏可打开检查器悬浮卡片（可拖动、可放大，不占第三列）。时间线显示轮次摘要；历史任务还能看权限审计。变更可审阅 Git 基线、文件分组和受限 Diff，不列出内部 commandId。仅最新完整写入型 Turn 在无漂移且经 Permission Broker 确认后可一键恢复上一轮文件；更早轮次只提供只读 Diff。不会执行 git reset / checkout / clean / stash。终端 / 产物目前仍是占位。
...
- 没有隔离 Worktree、用户交互终端或产物预览。Git 变更审阅已接入检查器「变更」标签；一键恢复上一轮文件仅覆盖最新完整写入型 Turn。
