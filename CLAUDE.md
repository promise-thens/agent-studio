...
- 当前计划：下一开发项为 Computer Use（对标 Codex）。说明书 [2026-09-17-computer-use-development.md](docs/superpowers/specs/2026-09-17-computer-use-development.md)。顺序：阶段 0 巩固内置页 → **阶段 0b 配套 Chrome 扩展 + 插件光标（P3-06）** → 阶段 1–4 macOS Helper 任意 App（P3-07）。五条 MUST 见说明书 §1。P0-19 GUI 走查仍开放但不挡开工。P1-06～08 与 P2 仍搁置。P3-05 已被 P0-21 取代。
- 状态：桌面定位为 Grok Build ACP Client，不自己当 Agent。设置现为供应商 / 外观 / 记忆 / Grok 配置 / 浏览器 / Hooks；插件页三栏为插件 / MCP / 技能，插件 tab 内分已安装 / 市场，不得打断运行中 Task。Inspector 现为 Timeline/Plan/Changes/Terminal/Artifacts，支持悬浮拖拽、右侧吸附后三列自适应与双栏审阅；Changes 与 Artifacts 打开时加宽为审阅工作区。宿主内置浏览器计划为工作区右栏，不新增 Inspector 标签。P2 Codex 暂缓
- 完成度：P0-01 至 P0-09、GACP-01 如前；GACP-02 核心已落地；P0-10 Task 1–4 已合入；P0-10A/B 代码已落地；P0-10C 代码已落地（自动验证已过，GUI 未跑）；库存扫描已补 `installed-plugins`；P0-10D 代码已落地（相关自动测试已过，开发版 GUI / TUI 对读未跑）；P0-10E 代码已落地（自动验证已过；already-configured 加源已修，开发版 GUI / 安装走查未跑）；P0-11 Task 1–4 已合入 main（自动验证已过，开发版 GUI 走查未跑）；GACP-03 代码已落地（真机/GUI 未收口）；P0-12 Task 1–4 代码已落地；P0-12A 两层审阅代码已落地（聚焦测试 + typecheck 已过，开发版 GUI 走查未跑）；P0-18 P0/P1 代码与关键 GUI 主路径已落地；P0-13 代码已落地（自动测试 + typecheck 已过，开发版 GUI 走查未跑）；GACP-04 代码已落地（相关 Vitest 已过；lifecycle/permission E2E 与 GUI 未跑）；P0-19g Task 1–3 代码已落地（接管默认关，Composer 三档 ask/assist/takeover，开发版 GUI 未走查）；P0-19a 任务 1–3 代码已落地（Grok 1.0.13 未广告 `plan`，当前 disabled-path；Vitest 已过；「开 Plan → 清单」GUI 未过）；GACP-06 代码已落地（可选 `parentId` 优先；无 parent 时只认 `[subagent:` spawn 行药丸卡；中文标题不聚类；开发版 GUI 未走查）；P0-19b 任务 1–3 代码已落地（Grok 沙箱四档 + 空闲 `--sandbox`；选择器等 `applied: true`；聚焦 Vitest 已过；开发版 GUI 未走查）；P0-19c 任务 1–3 代码已落地（rewind 冻结 not-advertised；两行预览卡；先文件后对话 executeTurnRewind；聚焦 Vitest 已过；开发版 GUI 未走查）；P0-19d 任务 1–2 代码已落地（Hooks 脱敏库存 + 设置页只读列表；聚焦 Vitest 已过；开发版 GUI 未走查）；P0-19e 任务 1–3 代码已落地（Timeline 后台徽章；停止走 `agent:cancel-turn`；证据仍 P0-11；Vitest 已过；开发版 GUI 未走查）；P0-19f 任务 1–5 代码已落地（browser L3、mapper 白名单、截图 Artifact、Composer HUD）；任务 6 overlay + 停止芯片已落地，**插件虚拟鼠标因 ACP 指针键 not-observed、`click_at` 为 viewport-css 不可映射而保持开放**（未发明光标）；自动验证已过；开发版 GUI 未走查，不得宣称计划完成；P0-21 任务 2–4 代码已落地（右栏壳、动作引擎、MCP 注入）；P0-21a 任务 1–4 代码已落地（可交互优先 snapshot、host-browser overlay 指针、HUD 文案）；任务 5 自动验证以本任务命令为准；开发版 GUI 未走查，不得宣称内置页光标可用或任意 App Computer Use 完成
- Grok ACP 加深：GACP-01 → GACP-02 → P0-10C/D/E → P0-11 → GACP-03（纳入 P0-19）→ GACP-04（方言契约已落地，不挡 P0-19）→ P0-19a～g 与 GACP-06（代码已落地扁平）。`available_commands_update` 现为 session 快照，不进 Timeline。命令证据走 `task:list-command-evidence` 等只读查询，不进交互终端。变更审阅走 `task:get-change-set` / `task:get-file-diff` / `task:list-turn-checkpoints`；文件恢复走 `task:preview-latest-turn-restore` / `task:restore-latest-turn`
- 兼容边界：首版仍只有一个执行槽；GROK_HOME 仍是 App 目录；记忆整棵树 junction 到 `~/.grok/memory`；用户 toml 只允许合并/删除 `[mcp_servers.*]`；`clientCapabilities: {}`；不把桌面做成 MCP Host 或 Marketplace Host。自动文件恢复仅 latest-turn 且无漂移；检查点只存哈希，禁止 git reset/checkout。完全接管（always-approve）按 P0-19g：当前 Task 显式开关，默认关，不写全局 yolo；确认后零确认卡，Grok 再问也由桌面 allow-once，禁止为此把 applied 打回 false（二次 toggle 会关掉接管）。Plan 开关只消费广告的 `name === 'plan'`，未广告不得伪造 `/plan`。子 Agent：`parentId` 或结构化 `[subagent:` 才成卡，禁止用中文标题猜树；不得宣称已支持任意协议嵌套。Grok 沙箱是内核 `--sandbox`，不是 Electron `webPreferences.sandbox`，也不是 Broker 替代；只读/严格不承诺挡住记忆 junction。后台命令停止走现有 `agent:cancel-turn`，不向用户 PTY 写 Ctrl+C，也不单停一条后台命令；输出进 P0-11 证据，不进 Inspector terminal。`browser` 已开 L3（once/task，grant 含 origin，写文件不得捎带）；`screen` / `clipboard` 仍 deny + unsupported，留给后置软件表面 / P3-07，不留给 19f。插件 overlay 是透明 `BrowserWindow`，不是 P0-21 `WebContentsView`，不读用户 Chrome Profile，不注入系统鼠标。停止芯片不等于虚拟鼠标。宿主页软件光标走 P0-21a 同一扇 overlay（`surface=host-browser`，`persistWhenUnfocused=false`）；插件 pointer 仍恒空；`computer-use` surface 本波无 producer
- 下一步：**按 Computer Use 说明书开发**，先 P3-06 配套扩展（用户选 tab、上报 CSS 盒+窗 DIP、overlay 画 `browser-plugin` 光标、hit-test 失败即停），再 P3-07 Helper（任意 Mac App、`elementIndex`、同一扇 overlay）。内置页 GUI 走查可并行。19f 已打开 `browser` L3，后到者不得改回 unsupported；货架 chrome-devtools 仍不许用不可映射的 `click_at` 发明光标。不要加回「继续任务」按钮。文件编辑器另开计划。未授权辅助功能、未做完五条 MUST，不得宣称已和 Codex 一样。
- 最近验证：2026-09-16 P0-21 overlay 准星与多屏。聚焦 agent-pointer-overlay / host-browser / plugin overlay / index 测试已过；typecheck / 目标 ESLint / `git diff --check` 已过；开发版 GUI 未走查。overlay 跟主窗所在屏，截图按 PNG IHDR 对齐 CSS；插件仍不画光标；无 CGEvent；不得宣称内置页光标可用或任意 App Computer Use 完成

<!-- gitnexus:start -->
# GitNexus — Code Intelligence

This project is indexed by GitNexus as **agent-studio** (9379 symbols, 24886 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
