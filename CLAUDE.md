...
- 当前计划：下一开发项为 Computer Use（对标 Codex）。说明书 [2026-09-17-computer-use-development.md](docs/superpowers/specs/2026-09-17-computer-use-development.md)。顺序：阶段 0 巩固内置页（Agent 自己开标签）→ **阶段 0b 独立浏览器设置页 + 配套扩展装上即同步并连接（P3-06，出厂开放）** → 阶段 1–4 macOS Helper 任意 App（P3-07）。五条 MUST 见说明书 §1。P0-19 GUI 走查仍开放但不挡开工。P1-06～08 与 P2 仍搁置。P3-05 已被 P0-21 取代。
...
- 下一步：**按 Computer Use 说明书开发**，先 P3-06（独立浏览器设置页、扩展装上即同步并连接、Agent 自己开标签；智能体权限出厂始终允许），再 P3-07 Helper（任意 Mac App、`elementIndex`、同一扇 overlay）。内置页 GUI 走查可并行。19f 已打开 `browser` L3，后到者不得改回 unsupported；货架 chrome-devtools 仍不许用不可映射的 `click_at` 发明光标。不要加回「继续任务」按钮。文件编辑器另开计划。未授权辅助功能、未做完五条 MUST，不得宣称已和 Codex 一样。
...
This project is indexed by GitNexus as **agent-studio** (9570 symbols, 25288 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
