...
- 下一步：**P3-07 macOS Helper**（任意 App、`elementIndex`、同一扇 overlay）。P3-06 开发版 GUI / 真 Chrome 走查可并行。19f 已打开 `browser` L3，后到者不得改回 unsupported；货架 chrome-devtools 仍不许用不可映射的 `click_at` 发明光标。不要加回「继续任务」按钮。文件编辑器另开计划。未授权辅助功能、未做完五条 MUST，不得宣称已和 Codex 一样。
